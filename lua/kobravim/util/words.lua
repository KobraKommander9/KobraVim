...
function M.enable()
	...
	local group = vim.api.nvim_create_augroup("kobravim_words", { clear = true })
...
function M.disable()
	...
	M.enabled = false
	vim.api.nvim_del_augroup_by_name("kobravim_words")
...
function M.jump(count, cycle)
	...
	if target then
		...
		if config.notify_jump then
			KobraVim.info(("Reference [%d/%d]"):format(idx, #words), { id = "kobravim.words.jump", title = "Words" })
		end
	...
	elseif config.notify_end then
		KobraVim.warn("No more references", { id = "kobravim.words.end", title = "Words" })
	end
end
