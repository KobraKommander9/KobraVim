...
M[#M + 1] = {
	...
	opts = function()
		return {
			...
			format = {
				-- formatting is handled by KobraVim
	...
	config = function(_, opts)
		...
		KobraVim.lsp.setup()
		KobraVim.lsp.on_dynamic_capability(require("kobravim.plugins.lsp.keymaps").on_attach)
		...
		if vim.fn.has("nvim-0.10") == 1 then
			if opts.inlay_hints.enabled then
				KobraVim.lsp.on_supports_method("textDocument/inlayHint", function(client, buffer)
					if
			...
			if opts.codelens.enabled and vim.lsp.codelens then
				KobraVim.lsp.on_supports_method("textDocument/codeLens", function(client, buffer)
